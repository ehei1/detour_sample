...
LPD3DXCONSTANTTABLE g_pTable = NULL;
...
VOID Cleanup()
{
    ...
    if (g_pTable)
        IDirect3D9_Release(g_pTable);

    if (g_pPixelShader)
        IDirect3DPixelShader9_Release(g_pPixelShader);
}
...
VOID Render()
{
    ...
    if (SUCCEEDED(IDirect3DDevice9_BeginScene(g_pd3dDevice)))
    {
        ...
        IDirect3DDevice9_SetPixelShader(g_pd3dDevice, g_pPixelShader);

        constexpr auto base_value = 1'000;
       
        // it will warn to console by unused variable in hlsl
        g_pTable->SetFloat(g_pd3dDevice, "unused", {});

        g_pTable->SetFloat(g_pd3dDevice, "base", static_cast<float>(base_value));
        g_pTable->SetFloat(g_pd3dDevice, "bias", static_cast<float>(::GetTickCount64() % base_value));
...
INT WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, LPWSTR, INT)
{
    ...
    auto detour_instance = std::make_unique<detour>();

    // Initialize Direct3D
    if (SUCCEEDED(InitD3D(hWnd)))
    {
        // initialize pixel shader and hook constant table of it
        {
            ComPtr<ID3DXBuffer> buffer;
            ComPtr<ID3DXBuffer> error;

            if (FAILED(D3DXCompileShaderFromFile(TEXT("ps.hlsl"), {}, {}, "main", "ps_3_0", {}, &buffer, &error, &g_pTable))) {
                auto message = static_cast<char*>(error->GetBufferPointer());

                OutputDebugStringA(message);
            }
            else if (FAILED(g_pd3dDevice->CreatePixelShader(reinterpret_cast<LPDWORD>(buffer->GetBufferPointer()), &g_pPixelShader))) {
                OutputDebugStringA("failed to create pixel shader");
            }

            detour_instance->hook(g_pTable);
        }
